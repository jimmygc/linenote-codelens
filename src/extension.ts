...
export const activate = async (context: vscode.ExtensionContext) => {
  ...
  context.subscriptions.push(
    ...
    vscode.commands.registerCommand("linenotecodelens.starNote",
            async (resource?: Entry, force_star: boolean = false) => {
        if(resource && (resource.type == LineNoteEntryType.Note ||
                        resource.type == LineNoteEntryType.StarNote))
        {
            let db = await getDB()
    ...
    vscode.commands.registerCommand("linenotecodelens.moveStarNote",
            async (resource?: Entry) => {
        vscode.commands.executeCommand("linenotecodelens.starNote", resource, true);
    }),
